  ...
  class MetricRollupAttributes < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SmplkitGeneratedClient::App::MetricRollupAttributes` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SmplkitGeneratedClient::App::MetricRollupAttributes`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @rollup.nil?
        invalid_properties.push('invalid value for "rollup", rollup cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @value !~ Regexp.new(/^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$/)
    ...
    # Custom attribute writer method with validation
    # @param [Object] rollup Value to be assigned
    def rollup=(rollup)
      if rollup.nil?
        fail ArgumentError, 'rollup cannot be nil'
      end
    ...
    def hash
      [name, value, unit, bucket, rollup].hash
    end
